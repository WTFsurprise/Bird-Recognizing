...
def generate_heatmap_base64(image_tensor, manager) -> str:
    """生成注意力热力图并返回Base64编码"""
    try:
        with torch.no_grad():
            image_tensor_device = image_tensor.to(manager.device)
            # 获取注意力矩阵
            attn_matrix = manager.engine._extract_attention_map(image_tensor_device)
            
            # 转换为热力图（使用OpenCV的火焰色系）
            attn_normalized = ((attn_matrix - attn_matrix.min()) / (attn_matrix.max() - attn_matrix.min() + 1e-8) * 255).astype(np.uint8)
            heatmap = cv2.applyColorMap(attn_normalized, cv2.COLORMAP_JET)
...
async def predict_with_visualization(file: UploadFile = File(...)):
    ...
    try:
        ...
        # 生成热力图
        heatmap_base64 = generate_heatmap_base64(image_tensor, manager)
